...
@section('title', '- Emails - View Selected Generic Email')
...
<section>
  <div class="container py-5">
    ...
    <h5>View Selected Generic Email</h5>
    {{-- title --}}
    ...
    <div class="row row-cols-1 row-cols-sm-4 pt-3">
      ...
      <div class="col pb-3">
        {{-- delete modal --}}
        {{-- modal button --}}
        <button type="button" class="btn btn-danger btn-block" data-toggle="modal" data-target="#trashModal">
          <i class="fas fa-trash-alt mr-2" aria-hidden="true"></i>Trash
        </button>
        {{-- modal button --}}
        {{-- modal --}}
        <div class="modal fade" id="trashModal" tabindex="-1" role="dialog" aria-labelledby="trashModalTitle" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="trashModalTitle">Trash</h5>
              ...
              <div class="modal-body">
                ...
                <form method="POST" action="{{ route('generic-emails.destroy', $selected_email->id) }}">
                  @method('DELETE')
                  @csrf
    ...
    <div class="table-responsive">
      <table class="table table-bordered table-fullwidth table-striped bg-white">
        ...
        <tbody>
          <tr>
            <td>
              @if (isset($selected_email->recipient_name))
                {{ $selected_email->recipient_name }}
              @else
                <p class="text-center">
            ...
            <td>{{ $selected_email->recipient_email }}</td>
            <td>{{ $selected_email->staff->getFullNameAttribute() }}</td>
            <td>{{ date('d/m/y - h:iA', strtotime($selected_email->created_at)) . ' (' . $selected_email->created_at->diffForHumans() . ')' }}</td>
          </tr>
        </tbody>
      </table>
    ...
    <div class="card">
      <div class="card-body pb-0">
        <p><b>{{ $selected_email->subject }}</b></p>
        <p>{!! nl2br($selected_email->text) !!}</p>
      </div> {{-- card-body --}}
    </div> {{-- card --}}
    {{-- email content --}}
