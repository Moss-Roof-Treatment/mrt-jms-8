...
@section('title', '- Reports - View Customer Referral Report')
                  ...
                  <tbody>
                    @foreach ($all_quotes as $quote)
                      <tr>
                        ...
                        <td>
                          <div class="progress">
                            <div class="progress-bar" role="progressbar" style="width: {{ number_format($quote->whereBetween('created_at', [$start_date, $end_date])->count() / $total_customer_referrals * 100) . '%' }}" aria-valuenow="{{ number_format($quote->whereBetween('created_at', [$start_date, $end_date])->count() / $total_customer_referrals * 100) }}" aria-valuemin="0" aria-valuemax="100">{{ number_format($quote->whereBetween('created_at', [$start_date, $end_date])->count() / $total_customer_referrals * 100) . '%' }}</div>
                          </div>
                        </td>
                        <td>{{ $quote->whereBetween('created_at', [$start_date, $end_date])->count() }}</td>
                        <td>{{ number_format($quote->whereBetween('created_at', [$start_date, $end_date])->count() / $total_customer_referrals * 100) . '%' }}</td>
                        <td>{{ $quote->whereIn('quote_status_id', [3, 4, 5, 6, 7])->whereBetween('created_at', [$start_date, $end_date])->count() }}</td>
                        <td>{{ number_format($quote->whereIn('quote_status_id', [3, 4, 5, 6, 7])->whereBetween('created_at', [$start_date, $end_date])->count() / $total_sold_quotes * 100) . '%' }}</td>
                      </tr>
                    @endforeach
                  </tbody>
                </table>
