  ...
  <div class="container py-5">
    ...
    <div class="row">
      ...
      <div class="col-sm-9">
        ...
        <h5 class="text-primary my-3"><b>All Customer Account Class Results</b></h5>

        <div class="card">
          <div class="card-body">
            <h4 class="text-primary text-center mb-4">Total Customer Account Classes: {{ $account_classes->count() }}</h4>
            ...
            @else
              @foreach ($account_classes as $account_class)
                <div class="row pb-3">
                  <div class="col-sm-4">
                    <p><b>{{ $account_class->first()->account_class->title }}</b> - {{ $account_class->count() }}</p>
                  </div> {{-- col-sm-4 --}}
                  <div class="col-sm-8">
                    <div class="progress my-1">
                      <div class="progress-bar" role="progressbar" style="width: {{ number_format($account_class->count() / $total_user_count * 100) . '%' }}" aria-valuenow="{{ number_format($account_class->count() / $total_user_count * 100) }}" aria-valuemin="0" aria-valuemax="100">{{ number_format($account_class->count() / $total_user_count * 100) . '%' }}</div>
                    </div>
                  </div> {{-- --}}
                </div> {{-- row --}}
              @endforeach
