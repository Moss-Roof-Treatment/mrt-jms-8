...
@section('title', '- Contractors - View All Contractors')
...
<section>
  <div class="container py-5">
    ...
    <div class="row row-cols-1 row-cols-sm-4 pt-3">
      ...
      <div class="col pb-3">
        <a href="{{ route('contractors.create') }}" class="btn btn-primary btn-block">
          <i class="fas fa-plus mr-2" aria-hidden="true"></i>Create New Contractor
        </a>
      </div> {{-- col pb-3 --}}
    ...
    {{-- contractors table --}}
    <h5 class="text-primary my-3"><b>All Contractors</b></h5>
    @if (!$all_contractors->count())
      <div class="card shadow-sm mt-3">
        <div class="card-body text-center">
          <h5>Threre are no contractors to display</h5>
        </div> {{-- card-body --}}
      </div> {{-- card --}}
    @else
      <div class="table-responsive mt-3">
        <table id="datatable" class="table table-bordered table-fullwidth table-striped bg-white" style="width:100%">
          ...
          <tbody>
            @foreach ($all_contractors as $user)
              <tr>
                <td>{{ $user->id }}</td>
                ...
                <td>
                  <a class="btn btn-primary btn-sm" href="{{ route('contractors.show', $user->id) }}">
