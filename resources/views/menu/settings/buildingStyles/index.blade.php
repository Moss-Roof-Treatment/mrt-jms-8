...
@section('title', '- Settings - All Building Styles')
...
<section>
  <div class="container py-5">
    ...
    <h5>View All Building Styles</h5>
    {{-- title --}}
    ...
    <div class="row pt-3">
      ...
      <div class="col-sm-3 pb-3">
        <a class="btn btn-primary btn-block" href="{{ route('building-style-settings.create') }}">
          <i class="fas fa-plus mr-2" aria-hidden="true"></i>Create New Building Style
        </a>
      </div> {{-- col-sm-3 pb-3 --}}
    </div> {{-- row pt-3 --}}
    {{-- navigation --}}

    <p class="text-primary my-3"><b>All Building Styles</b></p>
    @if (!$building_styles->count())
          ...
          <tbody>
            @foreach ($building_styles as $building_style)  
              <tr>
                ...
                <td>
                  @if ($building_style->description == null)
                  ...
                  @else
                    {{ substr($building_style->description, 0, 80) }}{{ strlen($building_style->description) > 80 ? "..." : "" }}
                  @endif
                </td>
                <td class="text-center">
                  @if ($building_style->is_editable == 0)
                    <a href="#" class="btn btn-primary btn-sm disabled" tabindex="-1" role="button" aria-disabled="true">
                      <i class="fas fa-edit mr-2" aria-hidden="true"></i>Edit
                    </a>
                  @else
                    <a href="{{ route('building-style-settings.edit', $building_style->id) }}" class="btn btn-primary btn-sm">
                      <i class="fas fa-edit mr-2" aria-hidden="true"></i>Edit
                    </a>
                  @endif
                  @if ($building_style->is_delible == 0)
                      ...
                      <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                          ...
                          <div class="modal-body">
                            ...
                            <form method="POST" action="{{ route('building-style-settings.destroy', $building_style->id) }}">
                              @method('DELETE')
                              @csrf
