...
@section('title', '- Settings - SWMS - View All SWMS Question Categories')
...
<section>
  <div class="container py-5">
    ...
    <h5>View All SWMS Question Categories</h5>
    {{-- title --}}
    ...
    <p class="text-primary my-3"><b>All SWMS Question Categories</b></p>
    @if (!$all_swms_question_categories->count())
      <div class="card shadow-sm mt-3">
        <div class="card-body text-center">
          <h5>There are no SWMS Question Categories to display</h5>
        </div> {{-- card-body --}}
      </div> {{-- card --}}
    @else
      <div class="table-responsive">
        <table id="datatable" class="table table-bordered table-fullwidth table-striped bg-white" style="width:100%">
          ...
          <tbody>
            @foreach ($all_swms_question_categories as $swms_question)  
              <tr>
                <td>{{ $swms_question->id }}</td>
                <td>{{ $swms_question->title }}</td>
                <td>{{ $swms_question->description }}</td>
                <td class="text-center">
                  <a href="{{ route('swms-questions-category-settings.edit', $swms_question->id) }}" class="btn btn-primary btn-sm">
