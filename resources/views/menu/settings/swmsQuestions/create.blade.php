...
@section('title', '- Settings - SWMS - Create New SWMS Question')
...
<section>
  <div class="container py-5">
    ...
    <h5>Create New SWMS Question</h5>
    {{-- title --}}
    ...
    <div class="row">
      <div class="col-sm-7">
        ...
        <p class="text-primary my-3"><b>Create New SWMS Question</b></p>

        <form action="{{ route('swms-questions-settings.store') }}" method="POST">
          @csrf

          <div class="form-group row">
            <label for="swms_question_category_id" class="col-md-2 col-form-label text-md-right">Category</label>
            <div class="col-md-9">
              <select name="swms_question_category_id" class="custom-select @error('swms_question_category_id') is-invalid @enderror mb-2">
                <option selected disabled>Please select a category</option>
                @foreach ($all_swms_question_categories as $category)
                  <option value="{{ $category->id }}" @if (old('swms_question_category_id') == $category->id) selected @endif>{{ $category->title }}</option>
                @endforeach
              </select>
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      ...
                      <a href="{{ route('swms-questions-settings.create') }}" class="btn btn-dark btn-block">
                        <i class="fas fa-undo-alt mr-2" aria-hidden="true"></i>Reset
                      </a>
                    </div> {{-- modal-body --}}
                  </div> {{-- modal-content --}}
