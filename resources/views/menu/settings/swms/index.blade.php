...
@section('title', '- Settings - SWMS - View All SWMS')
...
<section>
  <div class="container py-5">
    ...
    <p class="text-primary my-3"><b>All SWMS</b></p>
    @if (!$all_swms->count())
      <div class="card shadow-sm mt-3">
        <div class="card-body text-center">
          <h5>There are no SWMS to display</h5>
        </div> {{-- card-body --}}
      </div> {{-- card --}}
    @else
        ...
        <table id="datatable" class="table table-bordered table-fullwidth table-striped bg-white" style="width:100%">
          ...
          <tbody>
            @foreach ($all_swms as $swms)  
              <tr>
                <td>{{ $swms->id }}</td>
                <td>{{ $swms->quote->quote_identifier }}</td>
                <td>{{ $swms->tradesperson->getFullNameAttribute() }}</td>
                <td class="text-center">
                  <a href="{{ route('swms-settings.show', $swms->id) }}" class="btn btn-primary btn-sm">
