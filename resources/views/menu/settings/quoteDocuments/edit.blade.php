...
@section('title', '- Quote Documents - Edit Selected Quote Document')
...
<section>
  <div class="container py-5">
    ...
    <h5>Edit Selected Quote Document</h5>
    {{-- title --}}
    ...
    <div class="row">
      <div class="col-sm-7">
        ...
        <p class="text-primary my-3"><b>Edit Quote Document</b></p>

        <form action="{{ route('quote-document-settings.update', $selected_quote_document->id) }}" method="POST" enctype="multipart/form-data">
          @method('PATCH')
          @csrf
          ...
          <div class="form-group row">
            ...
            <div class="col-md-8">
              <input type="text" class="form-control @error('title') is-invalid @enderror mb-2" name="title" id="title" value="{{ @old('title', $selected_quote_document->title) }}" placeholder="Please enter the title" autofocus>
              @error('title')
            ...
            <div class="col-md-8">
              <select name="material_type_id" id="material_type_id" class="custom-select @error('material_type_id') is-invalid @enderror mb-2">
                ...
                @else
                  ...
                  @else
                    <option selected value="{{ $selected_quote_document->material_type_id }}">{{ $selected_quote_document->material_type->title }}</option>
                    <option disabled>Please select a task type</option>
                    <option value=""></option>
                  @endif
                  @foreach ($all_material_types as $material_type)
              ...
              <select name="task_type_id" id="task_type_id" class="custom-select @error('task_type_id') is-invalid @enderror mb-2">
                @if (old('task_type_id'))
                  ...
                  @foreach ($all_task_types as $task_type)
                    <option value="{{ $task_type->id }}" @if (old('task_type_id') == $task_type->id) selected @endif>{{ $task_type->title }}</option>
                  @endforeach
                @else
                  @if ($selected_quote_document->task_type_id == null)
                    <option selected disabled>Please select a task type</option>
                  @else
                    <option selected value="{{ $selected_quote_document->task_type_id }}">{{ $selected_quote_document->task_type->title }}</option>
                    <option disabled>Please select a task type</option>
                    <option value=""></option>
                  @endif
                  @foreach ($all_task_types as $task_type)
          ...
          <div class="form-group row">
            ...
            <div class="col-md-8">
              <select name="task_id" id="task_id" class="custom-select @error('task_id') is-invalid @enderror mb-2">
                @if (old('task_id'))
                  <option disabled>Please select a task</option>
                ...
                @else
                  ...
                  @else
                    <option selected value="{{ $selected_quote_document->task_id }}">{{ $selected_quote_document->task->title }}</option>
                    <option disabled>Please select a task</option>
                    <option value=""></option>
                  @endif
                  @foreach ($all_tasks as $task)
          ...
          <div class="form-group row">
            ...
            <div class="col-md-8">
              <textarea class="form-control @error('description') is-invalid @enderror mb-2" type="text" name="description" rows="5" placeholder="Please enter the description" style="resize:none">{{ old('description', $selected_quote_document->description) }}</textarea>
              @error('description')
                <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
                </span>
          ...
          <div class="form-group row">
            <div class="col-md-8 offset-md-3">
              <div class="custom-control custom-radio custom-control-inline">
                <input type="radio" id="isDefaultRadioInline1" name="is_default" class="custom-control-input @error('is_default') is-invalid @enderror mt-2" value="0" @if ($selected_quote_document->is_default == 0) checked @endif>
                <label class="custom-control-label" for="isDefaultRadioInline1">Is Not Default</label>
              </div>
              <div class="custom-control custom-radio custom-control-inline">
                <input type="radio" id="isDefaultRadioInline2" name="is_default" class="custom-control-input @error('is_default') is-invalid @enderror mt-2" value="1" @if ($selected_quote_document->is_default == 1) checked @endif>
          ...
          <div class="form-group row">
            <div class="col-md-8 offset-md-3">
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      ...
                      <a href="{{ route('quote-document-settings.edit', $selected_quote_document->id) }}" class="btn btn-dark btn-block">
                        <i class="fas fa-undo-alt mr-2" aria-hidden="true"></i>Reset
                      </a>
                    </div> {{-- modal-body --}}
                  </div> {{-- modal-content --}}
              ...
              {{-- modal --}}
              {{-- reset modal --}}
              <a href="{{ route('quote-document-settings.show', $selected_quote_document->id) }}" class="btn btn-dark">
